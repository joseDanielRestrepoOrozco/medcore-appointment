...
async function fetchFromUsers(
  ...
  if (!res.ok) {
    ...
    const err = new Error(
      `users service ${res.status} ${res.statusText} ${text}`
    );
...
export async function checkDoctorExists(
  ...
  try {
    ...
    return (
      user &&
      (user.role === 'MEDICO' || String(user.role).toUpperCase() === 'MEDICO')
    );
  } catch (err: any) {
...
export async function checkPatientExists(
  ...
  try {
    ...
    return (
      user &&
      (user.role === 'PACIENTE' ||
        String(user.role).toUpperCase() === 'PACIENTE')
    );
  } catch (err: any) {
    if (err.status === 404) return false;
    throw err;
