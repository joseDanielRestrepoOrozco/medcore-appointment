...
import { DEFAULT_TIMEZONE } from '../libs/config';
import redlock from '../libs/lock';
import { enqueueCalendarJob } from '../libs/queue';
...
export async function createAppointment(payload: CreateAppointmentPayload, authHeader?: string) {
  ...
  if (startLocal <= nowLocal) throw new Error('Cannot schedule appointments in the past');

  // Slot alignment: minutes should be 0 or 30
  const minute = startLocal.minute;
  if (!(minute === 0 || minute === 30)) throw new Error('startAt must be aligned to 30-minute slots (minutes must be 00 or 30)');
  ...
  if (process.env.SKIP_USER_VALIDATION === 'true') {
    ...
    [doctorExists, patientExists] = await Promise.all([
      ...
      usersClient.checkPatientExists(patientId, extraHeaders).catch(() => false),
    ]);
  }
  ...
  try {
    const availableSlots = await getAvailability(doctorId, startLocal.toISODate());
  ...
  try {
    ...
    if (doctorConflict) throw new Error('Appointment overlaps with an existing one for this doctor');
    ...
    if (patientConflict) throw new Error('Patient has a conflicting appointment in that time range');

    // Limit: max 3 upcoming active appointments per patient
    const activeStatuses = [AppointmentStatus.SCHEDULED, AppointmentStatus.CONFIRMED, AppointmentStatus.IN_PROGRESS];
    ...
    if (upcomingCount >= 3) throw new Error('Patient has reached the maximum number of active upcoming appointments (3)');
    ...
    try {
      await enqueueCalendarJob({ type: 'create', appointmentId: (created as any).id });
    } catch (e) {
    ...
    try {
      if (lock) {
        if (typeof (lock as any).release === 'function') await (lock as any).release();
        else if (typeof (lock as any).unlock === 'function') await (lock as any).unlock();
      }
    } catch (e) {
      // swallow
    }
...
export async function updateAppointmentStatus(id: string, status: string) {
  ...
  if (!allowed.includes(desired)) throw new Error(`Invalid state transition from ${current} to ${desired}`);

  const updated = await prisma.appointment.update({ where: { id }, data: { status: desired } as any });
  try {
...
export async function reprogramAppointment(id: string, newStartAt: string) {
  ...
  const zone = DEFAULT_TIMEZONE || 'America/Bogota';
  const nowLocal = DateTime.now().setZone(zone);
  const originalStartLocal = DateTime.fromJSDate((appt as any).startAt).setZone(zone);
  ...
  if (hoursBeforeOriginal < 24) throw new Error('Reprogramming must be requested at least 24 hours before the appointment');

  const newStartLocal = DateTime.fromISO(newStartAt, { zone });
  if (!newStartLocal.isValid) throw new Error('Invalid newStartAt date');
  if (newStartLocal <= nowLocal) throw new Error('New start must be in the future');

  // New start must align to 00 or 30
  const minute = newStartLocal.minute;
  if (!(minute === 0 || minute === 30)) throw new Error('newStartAt must be aligned to 30-minute slots (minutes must be 00 or 30)');
  ...
  if (doctorConflict) throw new Error('New time overlaps with existing appointment for this doctor');
  ...
  if (patientConflict) throw new Error('New time overlaps with existing appointment for this patient');
  ...
  const updated = await prisma.appointment.update({ where: { id }, data: { startAt: newStartUTC.toJSDate(), endAt: newEndUTC.toJSDate() } as any });
  try {
    await enqueueCalendarJob({ type: 'update', appointmentId: id });
  } catch (e) {
    console.warn('Could not enqueue calendar update job', e);
...
export async function confirmAppointment(id: string) {
  ...
  const appt = await prisma.appointment.findUnique({ where: { id } });
  if (!appt) throw new Error('Appointment not found');
  if ((appt as any).status === 'CANCELLED') throw new Error('Cannot confirm a cancelled appointment');
  const updated = await prisma.appointment.update({ where: { id }, data: { status: 'CONFIRMED' } as any });
  try {
    await enqueueCalendarJob({ type: 'update', appointmentId: id });
  } catch (e) {
    console.warn('Could not enqueue calendar update job', e);
...
export async function completeAppointment(id: string) {
  ...
  const appt = await prisma.appointment.findUnique({ where: { id } });
  if (!appt) throw new Error('Appointment not found');
  if ((appt as any).status === 'CANCELLED') throw new Error('Cannot complete a cancelled appointment');
  const updated = await prisma.appointment.update({ where: { id }, data: { status: 'COMPLETED' } as any });
  try {
    await enqueueCalendarJob({ type: 'update', appointmentId: id });
  } catch (e) {
    console.warn('Could not enqueue calendar update job', e);
...
export async function markNoShow(id: string) {
  ...
  const appt = await prisma.appointment.findUnique({ where: { id } });
  if (!appt) throw new Error('Appointment not found');
  if ((appt as any).status === 'CANCELLED') throw new Error('Cannot mark a cancelled appointment as no-show');
  const updated = await prisma.appointment.update({ where: { id }, data: { status: 'NO_SHOW' } as any });
  try {
    await enqueueCalendarJob({ type: 'update', appointmentId: id });
  } catch (e) {
    console.warn('Could not enqueue calendar update job', e);
